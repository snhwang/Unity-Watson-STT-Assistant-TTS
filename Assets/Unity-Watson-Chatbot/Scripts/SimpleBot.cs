...
public class SimpleBot : MonoBehaviour
{
    ...
    public IEnumerator CreateService()
    {
        ...
        if (string.IsNullOrEmpty(settings.Assistant_apikey.Trim()))
        {
        ...
        IamAuthenticator authenticator = new IamAuthenticator(apikey: settings.Assistant_apikey.Trim());

        //  Wait for tokendata
        while (!authenticator.CanAuthenticate())
            yield return null;

        Assistant_service = new AssistantService(settings.versionDate.Trim(), authenticator);
        if (!string.IsNullOrEmpty(settings.serviceUrl.Trim()))
        {
            Assistant_service.SetServiceUrl(settings.serviceUrl.Trim());
        }

        Assistant_service.CreateSession(OnCreateSession, settings.assistantId.Trim());
    ...
    public IEnumerator Welcome()
    {
        ...
        Assistant_service.Message(OnMessage, settings.assistantId.Trim(), sessionId);
        while (!messageTested)
        {
            messageTested = false;
            yield return null;
    ...
    public IEnumerator ProcessChat(string chatInput)
    {
        ...
        Assistant_service.Message(OnMessage, settings.assistantId.Trim(), sessionId, input: inputMessage);
    ...
    protected virtual void OnMessage(DetailedResponse<MessageResponse> response, IBMError error)
    {
        ...
        Debug.Log("response = " + response.Result.Output.Generic[0].Text);
        ...
        {
            textResponse = response.Result.Output.Generic[0].Text;
        }
